...
const useJobAnalysisStore = create((set, get) => ({
  stats: null,
  ...
  fetchList: async (params = {}, forceRefresh = false) => {
    ...
    const queryParams = {
      ...
      limit:
        params.limit !== undefined ? params.limit : pagination.itemsPerPage,
    ...
    try {
      const response = await api.get("/admin/jobs", { params: queryParams });
      if (response.success) {
        set({
  ...
  fetchUserDetails: async (userId, forceRefresh = false) => {
    ...
    try {
      const response = await api.get(`/admin/jobs/${userId}`);
      if (response.success) {
        set({
...
export default useJobAnalysisStore;
